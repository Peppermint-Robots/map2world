...
class MapConverter(Node):
    def __init__(self):
        ...
        self.declare_parameter("export_dir", str(Path.home()))
        ...
        self.threshold = self.get_parameter("occupied_threshold").get_parameter_value().double_value
    ...
    def contour_to_mesh(self, contour, metadata):
        ...
        for point in contour:
            ...
            if not mesh.is_volume:
                self.get_logger().info("Fixing mesh normals")
                mesh.fix_normals()
            meshes.append(mesh)
        mesh = trimesh.util.concatenate(meshes)
