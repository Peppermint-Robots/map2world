...
- Run map2world: `ros2 run map2world map2world --ros-args -p map_topic:={map_topic} -p mesh_type:={mesh_type} -p occupied_threshold:={occupied_threshold} -p box_height:={box_height} -p package_name:= {package_name} -p model_name:= {model_name} -p img_path:={img_path} -p map_mode:={map_mode} -p floor_padding:={floor_padding} -p red:={red} -p green:={green} -p blue:={blue}`
    - Parameters Reference:
        ...
        - floor_padding (Optional): Extra floor margin in meters added around all generated mesh bounds. Default Value: _50.0_
